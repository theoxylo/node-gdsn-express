module.exports = function (config) {
  ...
  function get_item_hierarchy_cin_promise(items) {
    return new Promise(function resolver(fulfill) {
      var receiver = items[0].recipient
      var sender = items[0].provider
      if (receiver != config.homeDataPoolGln) sender = config.homeDataPoolGln // dp must be sender or receiver
      var cin_xml = config.gdsn.create_cin(items, receiver, 'ADD', 'false', 'ORIGINAL', sender)
