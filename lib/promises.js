module.exports = function (config) {
  ...
  api.item_hierarchy_cin_validate = function (query, callback) {
    ...
    .then(function (result){ if (callback) callback(null, result); return})
    .catch(function (err)  { if (callback) callback(err); return         })
    .done()
  }
  ...
  function get_item_hierarchy_cin_promise(items) {
    return new Promise(function resolver(fulfill) {
      var cin_xml = config.gdsn.create_cin_28(items, config.homeDataPoolGln, 'ADD', 'false', 'ORIGINAL', items[0].provider)
