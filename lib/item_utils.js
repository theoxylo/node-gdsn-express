module.exports = function (config) {
  ...
  api.get_item_and_children = function (item, count, req_id, searched_gtins, callback) {
    ...
    trade_itemDb.getTradeItems(query, 0, 100, include_xml, function (err, children) { // only support up to 100 children per child
  ...
  api.get_item_and_parents = function (item, count, req_id, searched_gtins, callback) {
    ...
    trade_itemDb.getTradeItems(query, 0, 100, include_xml, function (err, parents) { // only support up to 100 children per child
      if (err) return callback(err)
