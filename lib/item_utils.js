module.exports = function (config) {
  ...
  api.fetch_all_children = function (item, req_id, callback) {
    this._get_item_and_children(item, 0, req_id, [], callback, /* include_xml */ false)
  }

  api.fetch_all_children_with_xml = function (item, req_id, callback) {
    this._get_item_and_children(item, 0, req_id, [], callback, /* include_xml */ true)
  }

  api._get_item_and_children = function (item, count, req_id, searched_gtins, callback, include_xml) {
    ...
    trade_item_db.getTradeItems(query, 0, 100, include_xml, function (err, children) { // only support up to 100 children per child
      ...
      children.forEach(function (child) {
        tasks.push(function (callback) {
          self._get_item_and_children(child, count + 1, req_id, searched_gtins, callback, include_xml)
        })
      })
  ...
  api.get_item_and_parents = function (item, count, req_id, searched_gtins, callback) {
    ...
    trade_item_db.getTradeItems(query, 0, 100, /* include_xml */ true, function (err, parents) { // only support up to 100 children per child
      if (err) return callback(err)
