module.exports = function (config) {
  ...
  api.send_from_dp = function (xml, callback, skip_biz) {
    ...
    db_msg_archive.saveMessage(xml, function (err, msg_info) {
      ...
      if (err) {
        log.info('Message saved to archive ERR in ' + (Date.now() - start) + ' ms before attempting as2: ' + err)
        return callback(err)
      }
      log.info('Message saved to archive in ' + (Date.now() - start) + ' ms before attempting as2: ' + msg_info.msg_id + ', modified: ' + new Date(msg_info.modified_ts))
    ...
    }, skip_biz) // end db_msg_archive.saveMessage
