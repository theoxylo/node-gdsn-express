module.exports = function (config) {
      ...
      msg.data.forEach(function (item) {
        tasks.push(function (task_done) {
          ...
          function (err, response, body) {
            ...
            var send_rci = getRciIsNeeded(body) // generate RCI to GR conditional upon DP API response
            if (send_rci) { 
              log.info('generating RCI for GR for GTIN ' + item.gtin)
              var rci_xml = config.gdsn.create_rci_to_gr(item, msg.status)
  ...
  function getRciIsNeeded(body) {
    ...
    try {
      ...
      console.log('ci api info.p_documentCommandHeader: ' + info.p_sendRciMsg)
      var result = (info.p_sendRciMsg === true || info.p_sendRciMsg == 'true' || info.send_rci == 'true')
      console.log('getRciIsNeeded result: ' + result)
      return result ? (info.p_documentCommandHeader || 'ADD') : false
    }
    catch (e) {
      log.debug('check rci error: ' + e)
    }
    return false
  }
