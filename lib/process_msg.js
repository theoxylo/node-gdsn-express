...
module.exports = function (config) {
  ...
  var log            = require('./Logger')('route_wf', config)
  var db_msg_archive = require('./db/msg_archive.js')(config)
  var db_trade_item  = require('./db/trade_item.js')(config)
  var db_party       = require('./db/trading_party.js')(config)
  ...
  api.workflow = function (msg, cic_state, callback) {
          ...
          function (err, response, body) {
            ...
            if (msg.msg_type == 'basicPartyRegistration') {
              ...
              db_msg_archive.saveMessage(bpr_xml, function (err, gen_msg_info) {
                ...
                api.send_by_as2(bpr_xml, config.gdsn_gr_gln)
              })
            }
    ...
    if (msg.msg_type == 'catalogueItemNotification') {
      ...
      if (msg.sender != msg.provider) { // sent by other SDP, so generate gS1Response AND cic RECEIVED if cic_state
        ...
        if (cic_state != 'DEFAULT') {
          var cic_xml = config.gdsn.populateRdpCicRecForSdpCin(config, msg, cic_state)
          db_msg_archive.saveMessage(cic_xml, function (err, new_cic) {
            ...
            api.send_by_as2(cic_xml, msg.sender)
          })
        }
        ...
        tasks.push(function (task_done) {
          ...
          function (err, response, body) {
            ...
            if (getRciIsNeeded(body)) { // generate RCI to GR conditional upon DP API response
              ...
              var rci_xml = config.gdsn.populateRciToGr(config, msg, item.gtin)
              db_msg_archive.saveMessage(rci_xml, function (err, rci_msg_info) {
                ...
                api.send_by_as2(rci_xml, config.gdsn_gr_gln)
              })
            }
          ...
          if (msg.sender == cic.recipient) {
            ...
            db_trade_item.findTradeItem(cic.recipient, cic.gtin, cic.provider, cic.tm, cic.tm_sub, function (err, item) {
              ...
              db_msg_archive.saveMessage(cic_xml, function (err, cic_msg_info) {
                ...
                api.send_by_as2(cic_xml, cic_msg_info.receiver)
              })
            })
            return
          }
    ...
    if (msg.msg_type == 'requestForCatalogueItemNotification') {
      ...
      if (msg.recipient == msg.sender) {
        ...
        var rfcin_xml = config.gdsn.populateRfcinToGr(config, msg)
        db_msg_archive.saveMessage(rfcin_xml, function (err, new_rfcin) {
          ...
          api.send_by_as2(rfcin_xml, new_rfcin.receiver)
        })
    ...
    if (msg.msg_type == 'catalogueItemHierarchicalWithdrawal') {
      ...
      if (msg.sender == msg.provider) { // forward from local DS to RDP
        ...
        db_party.findParty(msg.recipient, function (err, party) {
          ...
          var cihw_xml = config.gdsn.populateCihwToOtherSDP(config, msg)
          ...
          db_msg_archive.saveMessage(cihw_xml, function (err, new_cihw) {
            ...
            api.send_by_as2(cihw_xml, new_cihw.receiver)
          })
        })
      }
  ...
  function respond(error, msg, contentOwner, callback) {
    var response_xml = config.gdsn.populateResponseToSender(error && error.msg, config, msg, contentOwner)
    db_msg_archive.saveMessage(response_xml, function (err, saved_resp) {
      ...
      api.send_by_as2(response_xml, saved_resp.receiver)
      callback(null, 'response was generated and sent (if configured)')
    })
  }

  api.send_by_as2 = function (xml, receiver) {

    if (config.as2_gln_list.indexOf(receiver) == -1) {
      log.info('receiver ' + receiver + ' is not on as2 config list')
      return
    }
  ...
  api.send_by_as2_new = function (xml, receiver, done) {

    db_msg_archive.saveMessage(xml, function (err, msg_info) {
      if (err) {
        if (done) return done(err)
        log.info('Error saving message to archive before attempting as2: ' + err)
        return
      }
      log.info('Message saved to archive before attempting as2: ' + msg_info.msg_id + ', modified: ' + new Date(msg_info.modified_ts))

      if (config.as2_gln_list.indexOf(receiver) == -1) {
        if (done) return done(Error('receiver ' + receiver + ' is not on as2 config list'))
        log.info('receiver ' + receiver + ' is not on as2 config list')
        return
      }
