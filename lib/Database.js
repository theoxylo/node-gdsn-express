...
/////////////////////////// TRADE ITEM /////////////////////////////////
  ...
  this.findTradeItem = function (gtin, provider, tm, tmsub, recipient, cb) {
    if (!this.mdb) return cb(new Error('Database not connected'))
    try {
      var query = {
        gtin: gtin
      ...
      this.mdb.trade_items.find(query, {}, cb)
    }
    catch (err) {
      process.nextTick(function () {
        cb(err)
...
/////////////////////////// Message Archive /////////////////////////////////
  ...
  this.listMessages = function (cb) {
    if (!this.mdb) return cb(new Error('Database not connected'))
    try {
      this.mdb.archive.find({}, {xml:0}, cb)
    }
    catch (err) {
      process.nextTick(function () {
        cb(err)
  ...
  this.findMessage = function (instance_id, cb) {
    if (!this.mdb) return cb(new Error('Database not connected'))
    try {
      this.mdb.archive.find({instance_id: instance_id}, {xml:1}, cb)
    }
    catch (err) {
      process.nextTick(function () {
        cb(err)
