...
Database.prototype.saveTradeItem = function (item, cb) {
  ...
  this.archiveTradeItem(item, function (err, result) {
    ...
    self.mdb.trade_items.save(item, function (err, saved_item) {
      //log.debug('trade_items.save result: ' + JSON.stringify(saved_item))
      if (err) return cb(err)
      log.debug('Persisted trade item with GTIN ' + saved_item.gtin)
      cb(null, saved_item.gtin)
    })
  })
}
