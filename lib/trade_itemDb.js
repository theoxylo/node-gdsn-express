module.exports = function (config) {
	...
	var log   = require('./Logger')('tr_item_db', {debug: true})
...
//	  var self = this
	  this.archiveTradeItem(item, function (err, result) {
	    ...
	    config.database.mdb.trade_items.save(item, function (err, result) {
	...
	api.getTradeItems = function (query, page, perPage, includeXml, callback, count_only) {
	  ...
	  try {
	    ...
	    if (count_only) {
//	      var self = this
	      config.database.mdb.trade_items
	      .find(query, {})
	      .sort({modified_ts: -1})
	      .skip(max_count)
	      .limit(1)
	...
	api.archiveTradeItem = function (item, callback) {
	  ...
	  config.database.mdb.trade_items.update( { 
