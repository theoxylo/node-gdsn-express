module.exports = function (config) {
  ...
  api.listMessages = function (query, page, per_page, cb, include_all_fields, keep_history) {
    query = query || {}
    if (!keep_history) query.archived_ts = {$exists: false} 
    page = page || 0
    per_page = per_page || 5
  ...
  api.saveMessageInfo = function (msg_info, cb) {
    ...
    this.archiveMessageInfo(msg_info, function (err, archive_result) {
      ...
      // don't save linked objects to the msg_archive collection since we are persisting full xml,
      // and also have separate collections for those objects (parties, items, pubs, subs)
