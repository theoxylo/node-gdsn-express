module.exports = function (config) {
  ...
  api.saveMessage = function (msg, cb) {
    if (!msg) return cb(Error('msg_archive.saveMessage requires msg or raw xml argument'))

    if (!msg.xml && msg.length) { // passed msg is actually raw xml string 
      var orig_msg = msg
      msg = config.gdsn.get_msg_info(msg) // REPARSE
      if (!msg) return db(Error('failed to parse msg: ' + orig_msg))
    }
