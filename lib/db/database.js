exports.init = function (config) {
  ...
  var collections = ['msg_archive', 'trade_items', 'parties', 'logs', 'publications', 'subscriptions', 'confirmations']
  ...
  mdb.publications.ensureIndex({tm: 1})
  mdb.publications.ensureIndex({tm_sub: 1})

  mdb.subscriptions.ensureIndex({modified_ts: -1})
  mdb.subscriptions.ensureIndex({provider: 1})
  mdb.subscriptions.ensureIndex({recipient: 1})

  mdb.confirmations.ensureIndex({modified_ts: -1})
  mdb.confirmations.ensureIndex({provider: 1})
  mdb.confirmations.ensureIndex({recipient: 1})
  mdb.confirmations.ensureIndex({gtin: 1})
  mdb.confirmations.ensureIndex({tm: 1})
  mdb.confirmations.ensureIndex({tm_sub: 1})
  //mdb.confirmations.ensureIndex({recipient: 1, gtin: 1, provider: 1, tm: 1, tm_sub: 1, modified_ts: -1})

  log.info('connected to mongo db at ' + Date.now())
  if (log.is_debug) {
    for (var key in mdb) {
