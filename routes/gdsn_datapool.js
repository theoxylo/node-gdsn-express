...
module.exports = function (config) {
  ...
  api.post_to_gdsn = function(req, res, next) {
    ...
    if (msg_id) { // fetch existing msg xml and submit to dp
      log.debug('post_to_gdsn will use existing message with id ' + msg_id)
      msg_archive_db.findMessage(msg_id, function (err, messages) {
        ...
        log.info('post_to_gdsn xml length from msg archive lookup: ' + xml.length)
        log.info('post_to_gdsn xml setup (db) took ' + (Date.now() - start) + ' ms')
    ...
    else { // read posted content and submit to dp
      var xml = ''
