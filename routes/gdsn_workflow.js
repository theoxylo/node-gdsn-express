...
module.exports = function (config) {
  ...
  return function (req, res, next) {
    ...
    db_msg_archive.findMessage(sender, msg_id, function (err, msg) {
      ...
      if (msg.receiver != config.homeDataPoolGln) {
        return next(Error('to initiate workflow, the message receiver must be the datapool and not ' + msg.receiver))
      }
        ...
        if (msg.gtin) { // most reponses don't (yet) have a gtin! TODO
          ...
          function (err, response, body) {
            log.info('cin cirr api call took ' 
              ...
              + body)
            if (err) return next(err)
            if (response.statusCode != '200') return next(Error('bad status code ' + response.statusCode))
            if (!getSuccess(body)) return next(Error(body))
            if (!res.finished) {
              log.debug('response received to original msg_id ' + msg.request_msg_id + ' with status ' + msg.status)
              res.jsonp(body)
              res.end()
            }
          }) // end request.post
      ...
      if (msg.msg_type == 'basicPartyRegistration'
        ...
        async.parallel(tasks, function (err, results) {
          ...
          respond(err, msg, res, null, next) // contentOwner will be derived for each transaction in message by response generator
        }, 10) // concurrency
      ...
      if (msg.msg_type == 'catalogueItemNotification') {
        ...
        if (msg.sender != msg.provider) { // sent by other SDP, so generate gS1Response AND cic RECEIVED if state
          ...
          return respond(null, msg, res, msg.provider, next)
        }

        // CIN from local TP/publisher/DS
        // update local provider item in DP and post RCI to GR if needed
        if (msg.sender != msg.provider) {
          return respond(Error('msg sender CIN issue for gln: ' + msg.sender), msg, res, msg.provider, next)
        }
      ...
      if (msg.msg_type == 'catalogueItemSubscription') { // CIS cis
        ...
        async.parallel(tasks, function (err, results) {
          ...
          respond(err, msg, res, msg.recipient, next)
        }, 10) // concurrency
      ...
      if (msg.msg_type == 'requestForCatalogueItemNotification') {
        ...
        if (msg.sender != config.gdsn_gr_gln) return next(Error('incorrect RFCIN GR sender GLN: ' + msg.sender))
      ...
      if (msg.msg_type == 'catalogueItemHierarchicalWithdrawal') {
        ...
        if (msg.sender == msg.provider) { // forward from local DS to RDP
          ...
          db_party.findParty(msg.recipient, function (err, party) {
            ...
            if (err) return respond(err, msg, res, msg.provider, next)

            msg.recipient_dp = party.source_dp // registering home dp
          ...
          respond(null, msg, res, msg.provider, next)
        }
      } // end CIH
  ...
  function respond(error, req_msg_info, res, contentOwner, next) {
